...
#if defined(NDEBUG)
# error "Leafcoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Leafcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Leafcoin Signed Message:\n";
...
{
    // Leafcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
   ...
   unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
    ...
    // Leafcoin: This fixes an issue where a 51% attack can change difficulty at will.
    // Go back the full period unless it's the first retarget after genesis. Code courtesy of Art Forz
...
{
    // Leafcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LeafcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LeafcoinMiner : ProcessBlock, block not accepted");
    }
...
void static LeafcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running LeafcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LeafcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LeafcoinMiner, pwallet));
}
